...
def train_step_gru(
    ...
    rng: at.KeyArrayLike,
    state: training_utils.TrainState,
    batch: tuple[_model.Observation, _model.Actions, at.Array], # we have image_tokens preprocessed here in the batch
) -> tuple[training_utils.TrainState, dict[str, at.Array]]:
    model = nnx.merge(state.model_def, state.params)
    model.train()
    ...
    @at.typecheck
    def loss_fn(
        model: _model.BaseModel, rng: at.KeyArrayLike, observation: _model.Observation, actions: _model.Actions, image_tokens: at.Array
    ):
        batch_size = actions.shape[0]
        half = batch_size // 2 # first half of batch is positive samples, second half is negative samples
        ...
        # we are trying to train the filter to ignore unimportant information using negative samples of the past frames

        total_loss = (jnp.sum(pos_samples_loss) + jnp.sum(neg_samples_loss)) / batch_size
        return total_loss

    train_rng = jax.random.fold_in(rng, state.step)
    observation, actions, image_tokens = batch

    # Filter out frozen params.
    diff_state = nnx.DiffState(0, config.trainable_filter)
    loss, grads = nnx.value_and_grad(loss_fn, argnums=diff_state)(model, train_rng, observation, actions, image_tokens)
...
def main(config: _config.TrainConfig):
    ...
    ptrain_step = jax.jit(
        functools.partial(train_step_gru, config),
